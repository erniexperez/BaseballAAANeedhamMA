...
function updateActiveTeamButton(team) {
    document.querySelectorAll('#teamNav button').forEach(button => {
        button.classList.toggle('active', button.getAttribute('data-team') === team);
    });
}

function updateTeamHeader() {
    const team = getSelectedTeam();
    const header = document.getElementById('teamHomeHeader');
    if (team) {
        header.textContent = `${team} Home Page — Roster and Pitcher Status`;
    } else {
        header.textContent = 'Select a team to view roster and pitcher status';
    }
}

function updateAllStatuses() {
    document.querySelectorAll('.player').forEach(updateStatus);
}

function updateStatus(playerDiv) {
    const team = getSelectedTeam();
